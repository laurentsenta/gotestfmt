...
var stateMachine = []stateChange{
	...
	{
		regexp.MustCompile(`^ok\s+(?P<Package>[^\s]+)\s+(\((?P<Cached>cached)\)|(?P<Elapsed>[^\s]*))(|([\s]+)coverage: ((?P<Coverage>.*)% of statements|\[no statements]))( in (.*))?$`),
		stateRun,
		ActionPass,
		stateRun,
	},
	...
	{
		regexp.MustCompile(`^((?P<SomePackage>.*)\s+)?coverage: (?P<Coverage>.*)% of statements( in (.*))?$`),
		stateBetweenTests,
		ActionCoverage,
		stateBetweenTests,
	},
	...
	{
		regexp.MustCompile(`^((?P<SomePackage>.*)\s+)?coverage: \[no statements]$`),
		stateBetweenTests,
